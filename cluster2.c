...
char *readfile(char *filename, node_t ***arr, long long *vertices, long long *lines, long long *sum); 
long long cluster(node_t **mainarr, long long **clusterarr, long long vertices, long long lines); 
...
int main(int argc , char *argv[]) {
	...
	readfile(filename, &mainarr, &vertices, &lines, &sum);
...
*/
	spacing = cluster(mainarr, &clusterarr, vertices, lines);
	//spacing	= maxspacing(mainarr, clusterarr, lines);
	printf("total clusters are %lld %lld %lld \n", spacing , sum, spacing + sum);
}
...
long long cluster(node_t **mainarr, long long **clusterar, long long vertices, long long lines) {
...
char *readfile(char *filename, node_t ***arr, long long *vertices, long long *lines, long long *sum) {
	...
	if (getline(&lineptr, &n, fp) != -1) {
	   ...
	   *lines = atoi(strtok(NULL, LIMITER));
	   *sum = atoi(strtok(NULL, LIMITER));
